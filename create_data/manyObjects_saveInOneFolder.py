...
# set up global variables for counting radar objects per class
van = 0
car = 0
truck = 0
cyclist = 0
...
def get_radar_path():
    radar_path = '/media/moellya/yannick/raw_data/' + 'radar_-_snow_subsampled:2018-02-12_13-11-15:cam_stereo:left:image_rect:00390_1518437518841081632.json'
    return radar_path


def get_label_path():
    label_path = '/media/moellya/yannick/labels/' + 'snow_subsampled:2018-02-12_13-11-15:cam_stereo:left:image_rect:00390_1518437518841081632.json'
    return label_path
...
def write_file_for_PointNet(r_phiArray_subarray, amp_subarray, dop_subarray, identity):
    ...
    if identity == 'van':
        van += 1
        with open('data_dense_car/' + identity + '_' + '{0:04}'.format(van) + '.txt', 'w') as f:
            f.write(output)

    elif identity == 'car':
        car += 1
        with open('data_dense_car/' + identity + '_' + '{0:04}'.format(car) + '.txt', 'w') as f:
            f.write(output)

    elif identity == 'truck':
        truck += 1
        with open('data_dense_car/' + identity + '_' + '{0:04}'.format(truck) + '.txt', 'w') as f:
            f.write(output)

    elif identity == 'cyclist':
        cyclist += 1
        with open('data_dense_car/' + identity + '_' + '{0:04}'.format(cyclist) + '.txt', 'w') as f:
            f.write(output)

    elif identity == 'motorcycle':
        motorcycle += 1
        with open('data_dense_car/' + identity + '_' + '{0:04}'.format(motorcycle) + '.txt', 'w') as f:
            f.write(output)

    elif identity == 'bicycle':
        bicycle += 1
        with open('data_dense_car/' + identity + '_' + '{0:04}'.format(bicycle) + '.txt', 'w') as f:
            f.write(output)

    elif identity == 'person_sitting':
        person_sitting += 1
        with open('data_dense_car/' + identity + '_' + '{0:04}'.format(person_sitting) + '.txt', 'w') as f:
            f.write(output)

    elif identity == 'tram':
        tram += 1
        with open('data_dense_car/' + identity + '_' + '{0:04}'.format(tram) + '.txt', 'w') as f:
            f.write(output)

    elif identity == 'bus':
        bus += 1
        with open('data_dense_car/' + identity + '_' + '{0:04}'.format(bus) + '.txt', 'w') as f:
            f.write(output)

    elif identity == 'pedestrian':
        pedestrian += 1
        with open('data_dense_car/' + identity + '_' + '{0:04}'.format(pedestrian) + '.txt', 'w') as f:
            f.write(output)

    elif identity == 'noObject':
        noObject += 1
        with open('data_dense_car/' + identity + '_' + '{0:04}'.format(noObject) + '.txt', 'w') as f:
            f.write(output)
...
def look_in_far_radar(near, phileft_radar, phiright_radar, radar_data, identity):
    ...
    write_file_for_PointNet(r_phiArray_subarray, amp_subarray, dop_subarray, identity)
